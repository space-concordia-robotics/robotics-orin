...
public:
    ...
    SatelliteInfo(UbxDeserializer & reader) :
        ...
        azim        {reader.readI2()}, 
        prRes       {reader.readI2()}, 
        qualityInd  {reader.readX4(), reader.readXU(2, 0)}, 
        svUsed      {reader.readXF(3)}, 
        health      {reader.readXU(5, 4)}, 
        diffCorr    {reader.readXF(6)}, 
        smoothed    {reader.readXF(7)}, 
        orbitSource {reader.readXU(10, 8)}, 
...
public:
    static uint16_t PID = 0x3501; 
    const boolean   isPollingPacket; 
    // Deserializing constructor
    UbxPacketNavSat(const std::vector<uint8_t> & frame) :
        isPollingPacket {false}
    {
        UbxPacket(frame); 
        this->ensurePid(PID); 
